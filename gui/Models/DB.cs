...
namespace gui
{
    class DB
    {
        ...
        public bool updateCompanyWorkshopSchoolAssign(string WorkshopID, School School, string estimatedParticipants, string comments)
        {
            query = string.Format(@"
            UPDATE companyworkshop SET 
            WorkShop_School_ID={0}, WorkShop_School_Comments={1}, WorkShop_Number_Of_StudentPredicted={2},WorkShop_Status={3} WHERE WorkShop_ID={4};
                ", School, comments, estimatedParticipants, 4, WorkshopID);
            return Update(query);
        }
